...
class Post(db.Model):
    ...
    status = db.Column(db.Enum('public', 'private', 'draft', 'trash'), default="draft")
    categories = db.relationship('Category', secondary='post_category', backref='posts', lazy='select')
    time_created = db.Column(db.DateTime, nullable=False, default=db.func.now())
    time_updated = db.Column(db.DateTime, nullable=False, default=db.func.now(), onupdate=db.func.now())
